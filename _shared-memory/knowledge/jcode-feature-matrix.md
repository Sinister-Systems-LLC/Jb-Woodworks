...
| 12 | Memory-graph visualization | `tools/memory-graph-render/` → mermaid → `mermaid-rs-renderer` → PNG + Forge `panes/mermaid_panel.py` + RKOJ `/api/diagrams` | 🚧 PNG + Forge embed + REST API shipped, Mind web view pending | sanctum (render) + forge (Ctrl+D panel) + rkoj (REST API at :5077 — both shipped 2026-05-23) + mind (web view) | PNGs at `inventions/memory-graphs/`. Forge embed: `Ctrl+D` opens MermaidPanel listing newest renders + opens any on click. RKOJ Workstation API: `GET /api/diagrams` lists cached renders; `GET /api/diagrams/<stem>` serves PNG/SVG/HTML/MMD bytes with proper Content-Type (path-traversal guarded). Mind web view consumer is the last 🚧. |
