...
| 1c | `jcode usage` (token quota / billing) | `tools/sinister-usage/` v0.1.0 | ✅ shipped (env-check) | sanctum | 11-row endpoint registry mirroring sinister-login providers. 4 providers expose public per-key usage APIs (openai/copilot/fireworks/openai-compatible); 7 are console-only. Stdlib-only. NO network calls in v0.1.0 (canonical-11 wall); v0.2.0 will add `report` behind `--allow-network`. Soft cross-tool dep on sinister-login for the configured flag. CLI: `sinister usage list/check/check-all/matrix`. 12/12 unittests green. |
