...
# --- HELLO LEO welcome banner (operator hard-canonical 2026-05-25 ~02:35Z) ---
# Render the giant ASCII banner + 30-second key-points BEFORE the spawn UI so
# the operator (Leo or any new collaborator) sees a clear identity moment
# before the spawn logs scroll by. Composes with eve-welcome-banner.ps1.
$welcomeBanner = Join-Path $SanctumRoot 'automations\eve-welcome-banner.ps1'
if (Test-Path $welcomeBanner) {
    try {
        & $welcomeBanner -Name $OperatorName.ToUpper() -NoAnimation
    } catch {
        Write-Host ('  [warn] welcome banner threw: ' + $_.Exception.Message) -ForegroundColor DarkYellow
    }
}
...
$wizardPrompt = @"
You are the Sinister Setup Wizard. Your task is to onboard $OperatorName to the Sinister Sanctum fleet. BEFORE ANY OTHER OUTPUT, your FIRST visible message MUST be the output of `powershell -ExecutionPolicy Bypass -File $SanctumRoot\automations\eve-welcome-banner.ps1 -Name $($OperatorName.ToUpper()) -NoAnimation` (run via the Bash tool and surface the rendered ASCII banner verbatim). Then read $SanctumRoot\docs\LEO-WALKTHROUGH.md (overview of fleet usage) + $SanctumRoot\docs\LEO-SETUP.md + $SanctumRoot\docs\LEO-VAULT-SETUP.md so you know the full bring-up surface. Greet $OperatorName warmly + professionally; use small positive reinforcement after each successful step ("nice work, $OperatorName" / "good catch" / "perfect"); never condescending. Then walk $OperatorName through this checklist one item per turn, asking confirmation at each gate:
 ...
 12) Create ${OperatorName} first heartbeat at _shared-memory\heartbeats\$($OperatorName.ToLower()).json with agent='$OperatorName' + ts_utc + status='onboarding'.
...
Loop policy: loop=on. Ship one checklist item per turn; do not delete or kill anything without explicit '$OperatorName' confirmation; ask for permission before any write outside _shared-memory\setup\ and _shared-memory\heartbeats\.

When all items are DONE: (a) run `powershell -ExecutionPolicy Bypass -File $SanctumRoot\automations\eve-welcome-banner.ps1 -Name "$($OperatorName.ToUpper()) ONBOARDED" -NoAnimation -ShortMode` and surface the rendered output (the closing "LEO ONBOARDED" banner). (b) Print a clean 2-column summary table showing what was configured (claude CLI / OAuth / git config / branch / Docker / MCP / tasks / autonomy / vault / heartbeat) with PASS/SKIP/FAIL per row. (c) Drop a marker file at $SanctumRoot\_shared-memory\setup\$($OperatorName.ToLower())-welcome-completed-<utc>.json containing { operator, host, completed_at_utc, items_passed, items_skipped, items_failed }. (d) Write a final 'WELCOME TO THE FLEET' summary and end the turn cleanly.

If $OperatorName asks to abort at any point, write whatever was completed to the onboarding report and exit gracefully.
"@
