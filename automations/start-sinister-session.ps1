...
$BuiltinPhrases = @{
    'overview' = "$MemPreamble give me the project overview for <PROJECT> at <ROOT> (current state, pending TODOs, what is next). Use librarian.recall over 01_MEMORY/<PROJECT>/ if available. End the initial overview with: which lane I am in + 3-5 master-actionable next moves.$NoStopSuffix"
    'dev'      = "$MemPreamble we are working on <PROJECT> (root: <ROOT>). Read the project's .claude/memory/, SESSION-START.md, CLAUDE.md. Surface the top 3-5 master-actionable feature/fix candidates from MASTER-PLAN + PROGRESS + .claude/memory; pick the highest-priority one + BEGIN.$NoStopSuffix"
    'audit'    = "$MemPreamble audit <PROJECT> at <ROOT>. Use librarian.recall + auditor.run + git status. Surface: secrets at risk, stale TODOs, broken tests, push-readiness. TaskCreate one row per finding + walk them.$NoStopSuffix"
    'deploy'   = "$MemPreamble we are deploying <PROJECT> to production. Read the latest DEPLOY/HETZNER docs, confirm HEAD is clean + tagged, walk the deploy steps. Reversibility-gate any destructive op per canonical-11 (operator OK required before drop/force-push/kill).$NoStopSuffix"
    'push'     = "$MemPreamble push <PROJECT> to GitHub (<GITHUB>). Run secret-scrub first (MANDATORY). Then git add + commit (compose the message from the diff) + git push. Stop ONLY if secret-scrub fails or the push itself errors.$NoStopSuffix"
    'debug'    = "$MemPreamble debugging session on <PROJECT>. Read .claude/memory/ + living-mds/CURRENT-STATE.md + the latest BREAKTHROUGH-*.md. Identify the most-recent unresolved failure mode + claim it; if multiple are open, take the cheapest-first.$NoStopSuffix"
    'explore'  = "$MemPreamble open exploration on <PROJECT>. Read project root, .claude/memory/, docs/, NAVIGATION.md. Surface 3 surprising findings + TaskCreate one follow-up per finding.$NoStopSuffix"
