...
if [ -n "`${SINISTER_DRY_RUN:-}" ]; then
    # RKOJ-ELENO :: 2026-05-25 :: backtick-parse-fix. The resume phrase often
    # contains markdown backticks (e.g. forge-memory recall code-snippets).
    # Embedding `$bashPhrase` directly into a bash double-quoted printf arg
    # makes bash treat those backticks as command substitution at PARSE time,
    # which fails with "syntax error near unexpected token (" and bash exits
    # 2 BEFORE this branch even runs (parser doesn't know the branch won't
    # fire). mintty `--hold error` then parks the window with the phrase
    # visible. Avoided by not interpolating the phrase content here at all --
    # print a length-only placeholder, which is plenty for dry-run debug.
    printf '\n  > [DRY-RUN] resolved claude invocation:\n'
    printf '            powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$SanctumRoot\automations\claude-wrapper.ps1" -- --dangerously-skip-permissions <phrase: $($bashPhrase.Length) chars>\n'
    _claude_exit_code=0
...
"@
    ...
    try {
        if (Test-Path $minttyExe) {
            ...
            # RKOJ-ELENO :: 2026-05-25 :: headless-runners doctrine. Operator
            # 2026-05-25T11:47Z verbatim "these random powershells and cms
            # consoles that pop up have to stop. make a system with sinsiter
            # term to make them headless or something". Sub-E spawned 10 mintty
            # windows that visibly popped. Mitigation: minimize new mintty
            # windows by default (-w min) so they're spawned to taskbar instead
            # of foreground. The force-foreground SetForegroundWindow call
            # later (line ~2640) is gated on $env:SINISTER_FORCE_FOREGROUND;
            # default behavior now = headless minimize. Operator un-minimizes
            # the lane they want to interact with via taskbar click.
            #
            # TODO (Sinister Term spawn-as-tab API): when sinister-term.exe
            # ships, route new sessions through `sinister-term tab spawn`
            # instead of new mintty process. Stub cross-agent request written
            # to _shared-memory/cross-agent/ 2026-05-25T<utc>Z-sanctum-needs-
            # sinister-term-tab-api.md.
            $_minttyHeadless = @()
            if (-not $env:SINISTER_FORCE_FOREGROUND -or $env:SINISTER_FORCE_FOREGROUND -eq '0') {
                $_minttyHeadless = @('-w', 'min')
            }
            # RKOJ-ELENO :: 2026-05-25 (eve-exe lane iter-1) :: claude-icon resolution.
            # Operator hard-canonical 2026-05-25 "i need claude icon working asap".
            # Resolves to assets/claude-icon.ico generated by make-claude-icon.py. If
            # the asset is missing (fresh clone before first make-icon run), skip the
            # -i flag entirely so mintty uses its default icon rather than erroring.
            #
            # RKOJ-ELENO :: 2026-05-26 (eve-exe lane) :: BUG FIX -- mintty popup
            # "Could not load icon 'D:\Sinister': Access is denied." (operator Image #10).
            # Root cause: when PowerShell passes @('-i', 'D:\Sinister Sanctum\...ico')
            # to Start-Process -ArgumentList, mintty.exe's POSIX-style argv parser
            # splits the path on the space, parsing `-i D:\Sinister` then treating
            # `Sanctum\automations\...` as garbage -> the icon load fails on the
            # truncated path. Same family of PS 5.1 -> native exe argv-quoting bugs
            # documented above for Font=Cascadia and the -t window title.
            #
            # Fix: use mintty's --icon=<path> equals-sign form (a single argv element
            # mintty does NOT split on the embedded space). Mintty's getopt_long
            # accepts both `-i PATH` and `--icon=PATH`; the second form is one argv
            # token end-to-end so PowerShell's auto-quoting + mintty's parser agree.
            #
            # RKOJ-ELENO :: 2026-05-26 (sanctum lane iter-26 follow-up) :: BUG FIX #2 --
            # operator Image #6: title bar showed "Sanctum\automations\eve-launcher\
            # assets\claude-icon.ico --hold always -..." with Exit 126 "Failed to run
            # 'Sanctum\automations\...'". The --icon=PATH equals form STILL splits when
            # the embedded value path itself contains a space (PS 5.1 auto-quote heuristic
            # mis-parses --icon=D:\Sinister Sanctum\... as two args). Robust fix: prefer
            ...
            if (Test-Path $_claudeIconNoSpace) {
                ...
                # space, but degrading the spawn (no icon) is better than crashing.
                $_minttyIconArgs = @("--icon=$_claudeIcon")
            }
            $minttyArgs = $_minttyHeadless + $_minttyIconArgs + @(
