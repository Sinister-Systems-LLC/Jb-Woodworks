...
$BuiltinPhrases = @{
    'overview' = "$MemPreamble give me the project overview for <PROJECT> at <ROOT> (current state, pending TODOs, what is next). Use librarian.recall over 01_MEMORY/<PROJECT>/ if available. End with: which lane I am in + what you propose I work on."
    'dev'      = "$MemPreamble we are working on <PROJECT> (root: <ROOT>). Read the project's .claude/memory/, SESSION-START.md, CLAUDE.md. Then ask me what specific feature/fix we are tackling."
    'audit'    = "$MemPreamble audit <PROJECT> at <ROOT>. Use librarian.recall + auditor.run + git status. Surface: secrets at risk, stale TODOs, broken tests, push-readiness."
    'deploy'   = "$MemPreamble we are deploying <PROJECT> to production. Read the latest DEPLOY/HETZNER docs, confirm HEAD is clean + tagged, then walk me through the deploy steps."
    'push'     = "$MemPreamble push <PROJECT> to GitHub (<GITHUB>). Run secret-scrub first (MANDATORY). Then git add + commit (ask me for the message) + git push. Stop if anything fails."
    'debug'    = "$MemPreamble debugging session on <PROJECT>. Read .claude/memory/ + living-mds/CURRENT-STATE.md + the latest BREAKTHROUGH-*.md. Then ask me which failure mode I am chasing."
