...
**Source:** this folder is the canonical project root (real copy, migrated 2026-05-19 PM late from `C:\Users\Zonia\Desktop\Sinister Snap EMU.API`). Frozen backup of the prior state lives at `C:\Users\Zonia\Desktop\Sinister Snap EMU.API\` — untouched.
**Hub memory:** See [hub memory](../../../Sinister Skills/01_MEMORY/sinister-snap-emu/) — `SESSION-START.md` + `RESUME.md` + `WHERE-I-STOPPED.md`.

## One-line state

Pure-API workstream. 2026-05-19 EVE: Java-layer hook breakthrough — `SnapTokenApiGatewayHttpInterface.fetchSnapAccessTokens(KJh)` located + `UnifiedGrpcService.unaryCall` hooked. SSL_write path closed (Cronet's BoringSSL stripped). UA updated to `Snapchat/13.88.1.0`. Next: find concrete impl class of SnapTokenApiGateway and invoke fetchSnapAccessTokens directly, OR solve Step 4 UI block. See `source/living-mds/SESSION-LOG-2026-05-19-EVE.md`.

## How to work on this

Open `D:\Sinister\01_Projects\Sinister\Sinister-Snap-EMU\source\` in your editor (VSCode / etc.). The `source/` folder IS the canonical project root — every script, memory file, living-md, and `.git/` history is physically present at this D: location. Git, builds, bats, all paths inside the copy resolve to D:.

**Migration note:** see `source/living-mds/MIGRATION-2026-05-19.md` for the record (rollback procedure, what was excluded, etc.).
...
- Hub capsule: `..\..\..\Sinister Skills\03_PROJECTS\sinister-snap-emu.md`
